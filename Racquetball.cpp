    ...
    bool Racquetball::processUnbufferedInput(const Ogre::FrameEvent& evt)
    {
        ...
        if (true)
        {
            ...
            if(isClient)
            {
                ...
                serverScore = dest->serverScore;
                clientScore = dest->clientScore;
            }
        }
        ...
        for(int i = 0; i < numManifolds; i++) 
        {
            ...
            if(!isClient)
            {
                if((obA->getUserPointer() == ball->ballNode) && (obB->getUserPointer() == player->playerNode)) 
                {
                    ...
                    if((obA->getUserPointer() == ball->ballNode) && (obB->getUserPointer() == player2->playerNode2)){
                    int numContacts = contactManifold->getNumContacts();
                    for(int j=0; j<numContacts; ++j){
                        btManifoldPoint& pt = contactManifold->getContactPoint(j);
                        if(pt.getDistance()<0.0000f){
                            hitSecondPaddle = true;
                        }
                    }
                    }
                }
